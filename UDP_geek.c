...
fftw_complex* calculate(fftw_complex* data_calculate)
{
    ...
    for (i = 0; i < array_size_i; ++i) {
        for (j = 0; j < array_size_j; ++j) {
            data_calculate[array_size_i * j + i] = data_calculate[array_size_i * j + i] * exp(2 * M_PI * I * fnc
                                                                          * ta[array_size_j * i + j]);
        }
    }
    ...
    fftw_plan p = fftw_plan_many_dft(rank, n,  howmany, data_calculate, inembed, istride, idist,
